...
// Each event can be downcase/lowercase, or uppercase. If it is downcase/lowercase you need 1 coffee by events and if it is uppercase you need 2 coffees.a
...
function howMuchCoffee(events) {
  ...
  for (let char of events) {
    let checker = valid.includes(char.toLowerCase()) ? 1 : 0;
    if (checker) {
      total += char === char.toLowerCase() ? 1 : 2;
    }
  }
  return total <= 3 ? total : "You need extra sleep";
}
